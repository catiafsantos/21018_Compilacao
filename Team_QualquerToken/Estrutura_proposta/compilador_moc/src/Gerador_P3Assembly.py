...
class GeradorP3Assembly:
    ...
    def _pre_scan_quadruplos(self):
        ...
        for quad in self.quadruplos:
            ...
            if quad['op'] == 'write':
                arg1 = quad['arg1']
                is_literal = True  # Assumimos que é um literal até prova em contrário
                is_integer = True
                is_float = True
                try:
                    initial_val_float = int(arg1)
                    is_integer = True
                    is_float = False
                ...
                if is_literal:
                    if is_integer:
                        val = int(arg1)
                        # Divide um inteiro de 32 bits em duas palavras de 16 bits (baixa e alta)
                        low_word = val & 0xFFFF
                        high_word = (val >> 16) & 0xFFFF
                        # Usa os valores para inicializar a variável na secção de dados.
                        # A ordem WORD low, high é comum para arquiteturas little-endian.

                        self.string_literal_count += 1
                        var_name = f"INT_LT_{self.string_literal_count}"
                        self.string_literal_map[arg1] = var_name

                        self.data_section.append(self._format_line(
                            f"{var_name}_L", f"WORD", f"{low_word} ", f"; int '{var_name}' = {val}"))
                        self.data_section.append(self._format_line(
                            f"{var_name}_H", f"WORD", f"{high_word} ", f"; int '{var_name}' = {val}"))

                    if is_float:
                        val = float(arg1)
                        # SUCESSO: É um double literal.

                        self.string_literal_count += 1
                        var_name = f"FLOAT_LT_{self.string_literal_count}"
                        self.string_literal_map[arg1] = var_name
                        try:
                            # 1. Separar a parte inteira da fracionária
                            parte_inteira = int(val)
                            parte_fracionaria = val - parte_inteira

                            # 2. Definir a precisão e converter a parte fracionária para um inteiro.
                            #    Usamos 10000 para até 4 casas de precisão. O round() ajuda com
                            #    imprecisões de ponto flutuante.
                            precisao = 10000
                            parte_fracionaria_int = int(round(parte_fracionaria * precisao))

                            # 3. Dividir cada parte (que tratamos como um inteiro de 32 bits)
                            #    em duas palavras de 16 bits cada.

                            # Palavras da parte inteira
                            int_low_word = parte_inteira & 0xFFFF
                            int_high_word = (parte_inteira >> 16) & 0xFFFF

                            # Palavras da parte fracionária
                            frac_low_word = parte_fracionaria_int & 0xFFFF
                            frac_high_word = (parte_fracionaria_int >> 16) & 0xFFFF

                            # 4. Gerar as diretivas WORD para a secção de dados, uma por linha.
                            #    A etiqueta é colocada na primeira linha para marcar o início do bloco.
                            self.data_section.append(self._format_line(
                                f"{var_name}", f"WORD", f"{int_low_word}", f"; double (ponto fixo) '{val}' - parte inteira (low)"))
                            self.data_section.append(self._format_line(
                                f"{var_name}2", f"WORD", f"{int_high_word}", f"; parte inteira (high)"))
                            self.data_section.append(self._format_line(
                                f"{var_name}3", f"WORD", f"{frac_low_word}", f"; parte fracionária (low)"))
                            self.data_section.append(self._format_line(
                                f"{var_name}4", f"WORD", f"{frac_high_word}", f"; parte fracionária (high)"))

                        except Exception as e:
                            # Fallback em caso de erro na conversão
                            self.data_section.append(f";; ERRO ao processar o double literal {val}: {e}")
                            # Ainda assim, alocar espaço por defeito
                            self.data_section.append(f"{var_name}: WORD 0 ; double '{val}' - erro")
                            self.data_section.append(f"          WORD 0")
                            self.data_section.append(f"          WORD 0")
                            self.data_section.append(f"          WORD 0")
                ...
                if not is_literal:
                    ...
                    print(f"'{arg1}' é uma variável.")
                    # AQUI É CRUCIAL: Você não sabe o tipo apenas pelo nome.
                    # Você precisa consultar a sua Tabela de Símbolos (criada na análise semântica)
                    # para descobrir o tipo da variável 'arg1' e então chamar a rotina correta.
                    # Ex: tipo = tabela_simbolos.get_type(arg1)
                    #     if tipo == 'double': call_write_double()
                    #     else: call_write_int()
    ...
    def translate_tac_instruction(self, instr, quad_num):
        ...
        # Entrada/Saída (exemplo: print)
            ...
            str_label = self.string_literal_map.get(arg1.strip('"'))
            sufixo=""
            if str_label.startswith('INT'):
                sufixo = "INT"
                str_label=str_label+"_L"
            if str_label.startswith('FLOAT'):
                sufixo = "FLT"
            ...
            self.assembly_code.append(self._format_line(f"; {op.lower()} {arg1_label}", "", "-" * 25))
            self.assembly_code.append(
                self._format_line("", "PUSH", f"{str_label}", "; Endereço do valor passado via pilha"))
            self.assembly_code.append(self._format_line("", "CALL", f"{op.upper()}_{sufixo}", "; Chama a rotina"))
            self.assembly_code.append(self._format_line("", "POP", "R0"))
            self.assembly_code.append("")
            if (sufixo=="FLT"):
                self.add_function_write_float()
                self.add_function_write_int_routine()
            else:
    ...
    def add_function_write_float(self):
        """
        Adiciona a sub-rotina WRITE_DOUBLE ao código assembly.

        Esta é uma implementação que imprime um número em formato de PONTO FIXO.
        Ela assume que o valor double de 64 bits foi previamente convertido para:
          - 32 bits para a parte inteira (as duas primeiras palavras).
          - 32 bits para a parte fracionária (as duas últimas palavras).

        Recebe o ENDEREÇO da variável (4 palavras) na pilha.
        """
        if 'write_double' not in self.declared_functions:
            self.declared_functions.add('write_double')
            self.assemblyfunction_code.append("")
            self.assemblyfunction_code.append(
                "; write_double(endereco): Imprime um número de ponto fixo (simplificado).")
            self.assemblyfunction_code.append(self._format_line("WRITE_FLT:", "NOP"))

            # --- Etapa 1: Obter o endereço e imprimir a parte INTEIRA ---
            self.assemblyfunction_code.append(
                self._format_line("", "MOV", "R1, M[SP+2]", "; R1 = Endereço da variável"))

            # AVISO: Esta rotina simplificada usa apenas os 16 bits baixos da parte inteira.
            # Uma implementação completa exigiria uma rotina de impressão de 32 bits.
            self.assemblyfunction_code.append(
                self._format_line("", "MOV", "R2, M[R1]", "; R2 = Parte baixa da parte inteira"))

            self.assemblyfunction_code.append(
                self._format_line("", "PUSH", "R2", "; Passa a parte inteira para a rotina de impressão de inteiros"))
            self.assemblyfunction_code.append(
                self._format_line("", "CALL", "WRITE_INT_RTN", "; Chama rotina auxiliar para imprimir um inteiro"))
            self.assemblyfunction_code.append(self._format_line("", "POP", "R0", "; Limpa a pilha"))

            # --- Etapa 2: Imprimir o ponto decimal ---
            self.assemblyfunction_code.append(self._format_line("PRINT_DECPOINT:", "MOV", "R4, '.'"))
            self.assemblyfunction_code.append(self._format_line("", "MOV", "M[FFFEh], R4", "; Imprime o ponto decimal"))

            # --- Etapa 3: Imprimir a parte FRACIONÁRIA ---
            # AVISO: Usa apenas os 16 bits baixos da parte fracionária.
            self.assemblyfunction_code.append(
                self._format_line("", "MOV", "R2, M[R1+2]", "; R2 = Parte baixa da parte fracionária"))

            # Reutiliza a rotina de impressão de inteiros para a parte fracionária.
            self.assemblyfunction_code.append(
                self._format_line("", "PUSH", "R2", "; Passa a parte fracionária para a rotina de impressão"))
            self.assemblyfunction_code.append(
                self._format_line("", "CALL", "WRITE_INT_RTN", "; Chama rotina auxiliar novamente"))
            self.assemblyfunction_code.append(self._format_line("", "POP", "R0", "; Limpa a pilha"))

            # --- Etapa 4: Fim da rotina ---
            self.assemblyfunction_code.append(self._format_line("WRITE_FLT_END:", "RET"))
            self.add_function_write_int_routine()  # Garante que a rotina auxiliar é incluída

    def add_function_write_int_routine(self):
        """
        Rotina auxiliar que imprime um único inteiro de 16 bits.
        Recebe o valor via pilha. Usada por WRITE_DOUBLE.
        """
        if 'write_int_routine' not in self.declared_functions:
            self.declared_functions.add('write_int_routine')
            self.assemblyfunction_code.append("")
            self.assemblyfunction_code.append("; WRITE_INT_ROUTINE: Imprime um inteiro de 16 bits passado na pilha.")
            self.assemblyfunction_code.append(self._format_line("WRITE_INT_RTN:", "NOP"))

            self.assemblyfunction_code.append(
                self._format_line("", "MOV", "R2, M[SP+2]", "; R2 = Valor do inteiro a imprimir"))
            self.assemblyfunction_code.append(
                self._format_line("", "MOV", "R5, 0", "; R5 = Contador de dígitos"))

            # Tratar o caso de o número ser zero
            self.assemblyfunction_code.append(self._format_line("", "CMP", "R2, 0"))
            self.assemblyfunction_code.append(
                self._format_line("", "JMP.NZ", "WI_CONV_LOOP", "; Se não for zero, começa a conversão"))
            ...
            self.assemblyfunction_code.append(self._format_line("", "JMP", "WI_END"))

            # Loop de conversão: extrai dígitos por divisão sucessiva
            self.assemblyfunction_code.append(self._format_line("WI_CONV_LOOP:", "MOV", "R4, 10"))
            self.assemblyfunction_code.append(
            ...
            self.assemblyfunction_code.append(self._format_line("", "CMP", "R2, 0"))
            self.assemblyfunction_code.append(
                self._format_line("", "JMP.NZ", "WI_CONV_LOOP", "; Repete se o quociente não for zero"))

            # Loop de impressão: retira dígitos da pilha e imprime
            self.assemblyfunction_code.append(
                self._format_line("WI_PRT_LOOP:", "POP", "R3", "; Retira dígito da pilha"))
            self.assemblyfunction_code.append(
            ...
            self.assemblyfunction_code.append(
                self._format_line("", "JMP.NZ", "WI_PRT_LOOP", "; Repete até todos os dígitos serem impressos"))

            self.assemblyfunction_code.append(self._format_line("WI_END:", "RET"))
    ...
    def add_function_write(self):
        ...
        if 'write' not in self.declared_functions:
            ...
            self.assemblyfunction_code.append(self._format_line("WRITE_INT:", "NOP"))
    ...
    def add_function_write_float_old(self):
        """
        Adiciona a sub-rotina WRITE_DOUBLE ao código assembly.

        Esta é uma implementação SIMPLIFICADA que imprime um valor double,
        assumindo que ele pode ser tratado como um número de ponto fixo para
        extrair as partes inteira e fracionária. Uma implementação completa
        e precisa exigiria uma biblioteca de matemática IEEE 754 completa.

        Recebe o ENDEREÇO da variável double (4 palavras) na pilha.
        """
        if 'write_double' not in self.declared_functions:
            self.declared_functions.add('write_double')
            self.assemblyfunction_code.append("")
            self.assemblyfunction_code.append("; write_double(endereco_da_variavel): Imprime um número double (simplificado).")
            self.assemblyfunction_code.append(self._format_line("WRITE_DOUBLE:", "NOP"))

            # --- Etapa 1: Obter o endereço e carregar as 4 palavras do double ---
            # Por simplicidade, esta rotina irá operar apenas com a parte mais significativa,
            # tratando-a como a parte inteira.
            self.assemblyfunction_code.append(
                self._format_line("", "MOV", "R1, M[SP+2]", "; R1 = Endereço da variável double"))
            self.assemblyfunction_code.append(
                self._format_line("", "MOV", "R2, M[R1]", "; R2 = Palavra mais significativa (parte inteira aproximada)"))
            # Nota: Uma rotina real carregaria todas as 4 palavras e decodificaria o formato IEEE 754.

            # --- Etapa 2: Imprimir a parte inteira (reutilizando a lógica de WRITE_INT) ---
            # O código a seguir é uma adaptação da rotina de impressão de inteiros.
            self.assemblyfunction_code.append(
                self._format_line("", "CLR", "R5", "; R5 = Contador de dígitos"))

            # Tratar sinal
            self.assemblyfunction_code.append(self._format_line("", "CMP", "R2, 0"))
            self.assemblyfunction_code.append(
                self._format_line("", "JMP.NN", "WD_POS", "; Salta se for positivo ou zero"))
            self.assemblyfunction_code.append(self._format_line("", "MOV", "R3, '-'"))
            self.assemblyfunction_code.append(self._format_line("", "MOV", "M[FFFEh], R3"))
            self.assemblyfunction_code.append(self._format_line("", "NEG", "R2"))

            self.assemblyfunction_code.append(self._format_line("WD_POS:", "CMP", "R2, 0"))
            self.assemblyfunction_code.append(
                self._format_line("", "JMP.NZ", "WD_CONV_LOOP", "; Se não for zero, converte"))
            # Se for zero, imprime '0' e avança para a parte decimal
            self.assemblyfunction_code.append(self._format_line("", "MOV", "R3, '0'"))
            self.assemblyfunction_code.append(self._format_line("", "MOV", "M[FFFEh], R3"))
            self.assemblyfunction_code.append(self._format_line("", "JMP", "PRINT_DECPOINT"))

            # Loop de conversão da parte inteira
            self.assemblyfunction_code.append(self._format_line("WD_CONV_LOOP:", "MOV", "R4, 10"))
            self.assemblyfunction_code.append(
                self._format_line("", "DIV", "R2, R4", "; R2=Quociente, R4=Resto"))
            self.assemblyfunction_code.append(self._format_line("", "PUSH", "R4"))
            self.assemblyfunction_code.append(self._format_line("", "INC", "R5"))
            self.assemblyfunction_code.append(self._format_line("", "CMP", "R2, 0"))
            self.assemblyfunction_code.append(
                self._format_line("", "JMP.NZ", "WD_CONV_LOOP"))

            # Loop de impressão da parte inteira
            self.assemblyfunction_code.append(
                self._format_line("WD_PRT_LOOP:", "POP", "R3"))
            self.assemblyfunction_code.append(self._format_line("", "ADD", "R3, '0'"))
            self.assemblyfunction_code.append(self._format_line("", "MOV", "M[FFFEh], R3"))
            self.assemblyfunction_code.append(self._format_line("", "DEC", "R5"))
            self.assemblyfunction_code.append(
                self._format_line("", "JMP.NZ", "WD_PRT_LOOP"))

            # --- Etapa 3: Imprimir a parte decimal ---
            self.assemblyfunction_code.append(self._format_line("WD_PRINT_DECIMAL_POINT:", "MOV", "R3, '.'"))
            self.assemblyfunction_code.append(self._format_line("", "MOV", "M[FFFEh], R3", "; Imprime o ponto decimal"))

            # AVISO: A lógica abaixo é um placeholder. Uma implementação real
            # exigiria aritmética de ponto flutuante simulada para extrair a parte fracionária.
            # Por simplicidade, vamos imprimir um número fixo de zeros.
            self.assemblyfunction_code.append(self._format_line("", "MOV", "R3, '0'"))
            self.assemblyfunction_code.append(self._format_line("", "MOV", "M[FFFEh], R3", "; Imprime decimal 1"))
            self.assemblyfunction_code.append(self._format_line("", "MOV", "M[FFFEh], R3", "; Imprime decimal 2"))
            self.assemblyfunction_code.append(self._format_line("", "MOV", "M[FFFEh], R3", "; Imprime decimal 3"))
            self.assemblyfunction_code.append(self._format_line("", "MOV", "M[FFFEh], R3", "; Imprime decimal 4"))

            # --- Etapa 4: Fim da rotina ---
            self.assemblyfunction_code.append(self._format_line("WRITE_DOUBLE_END:", "RET"))
